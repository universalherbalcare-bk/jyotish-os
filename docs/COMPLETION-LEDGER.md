...
| Branch protection | BLOCKED (GitHub plan: 403 "Upgrade to GitHub Pro or make this repository public") — owner decision: Pro / public / stay private with PR-only main as convention (named risk) | vendor pins exact-match and `--check` OK; actionlint 0 findings; `scripts/github-bootstrap.sh --dry-run` prints create/push + protection payload; no remote exists |
