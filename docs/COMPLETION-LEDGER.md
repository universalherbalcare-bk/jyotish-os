...
| Hosted CI + branch protection | BLOCKED (owner: push confirm) | vendor pins exact-match and `--check` OK; actionlint 0 findings; `scripts/github-bootstrap.sh --dry-run` prints create/push + protection payload; no remote exists |
